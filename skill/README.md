...
This skill lets OpenClaw agents use **letheClaw** (strategic memory for AI agents): store and search memories, send criticality signals, load recent corrections, mark operator corrections, and read provenance.
...
- **Load recent corrections** (what went wrong last — provenance-based, not tag heuristics).
- **Send criticality signals** (`failure`, `success`, `referenced`) — no raw numbers.
